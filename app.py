...
heroes = {
    "player": PlayerUnit('Игрок', unit_class=WarriorClass),
    "enemy": EnemyUnit('Противник', unit_class=ThiefClass)
}
...
@app.route("/fight/hit")
def hit():
    '''кнопка нанесения удара обновляем экран боя (нанесение удара) (шаблон fight.html)
...
def pass_turn():
    '''кнопка пропуск хода логика практикчески идентична предыдущему эндпоинту
...
def choose_hero():
    ...
    if request.method == 'GET':
        ...
        return render_template('hero_choosing.html', result={'header': header, 'weapons': weapons,
                                                             'armors': armors, 'classes': classes})
